...
class Students(models.Model):
    ...
    id = models.AutoField(primary_key=True)
    admin = models.OneToOneField(CustomUser, on_delete=models.CASCADE, unique=True)
    ...
    address = models.TextField()
    subjects = models.ManyToManyField(Subjects, blank=True, related_name='students')
...
def create_user_profile(sender, instance, created, **kwargs):
    if created:
        if instance.user_type == 1:  # HOD
            AdminHOD.objects.create(admin=instance)  # Create AdminHOD instance
                ...
                with transaction.atomic():
                    ...
                    session_year, _ = SessionYearModel.objects.get_or_create(
                        defaults={
                            ...
                            'session_end_year': timezone.now().date() + timedelta(days=365)
                        }
                    )
                    
                    # Only create student if one doesn't already exist
